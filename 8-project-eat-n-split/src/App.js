...
export default function App() {
  const [showAddFreinds, setShowAddFreinds] = useState(false);
  const [friends, setFriends] = useState(initialFriends);
  const [selectedFriend, setSelectedFriend] = useState(null);
  function handleShowAddFriend() {
    setShowAddFreinds(!showAddFreinds);
  }
  ...
  return (
    <div className="app">
      <div className="sidebar">
        ...
        />
        {showAddFreinds ? (
          <FormAddFriend onAddFriend={handelAddFriend} friends={friends} />
        ) : (
          ""
        )}
        <Button onCLick={handleShowAddFriend}>
          {showAddFreinds ? "Close " : "Add Friends"}
        </Button>
      </div>
      {selectedFriend !== null ? (
        <FormSplitBill
...
function FormAddFriend({ onAddFriend }) {
  ...
  return (
    <form className="form-add-friend" onSubmit={handleSubmit}>
      ...
      <input type="text" onChange={handleAddImage} value={image} />
      <Button>add</Button>
    </form>
  );
}
